...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                # logger.debug('body_part.x:', body_part.x)
                # logger.debug('body_part.y:', body_part.y)
                # logger.debug('image_w:', image_w)
                # logger.debug('image_h:', image_h)
                centers[i] = center
    ...
    def inference(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        # myHeatMap = self.persistent_sess.run([self.tensor_heatMat], feed_dict={
        #         self.tensor_image: [img], self.upsample_size: upsample_size
        #     })
        #print('wf---------------------------------->>>')
        #print(peaks)
